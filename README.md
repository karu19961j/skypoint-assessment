...
| Pipeline filters                 | Left sidebar: skills (any/all), experience range, current/expected CTC ceiling, max notice (discrete buckets: Immediate / 15 / 30 / 60 / 90), applied-date range, stage, keyword search across cover note, skills, **and the candidate's resume text**, plus four sort modes (recent, lowest expected CTC, shortest notice, most experienced). |
...
| Profile / notes drawer           | **View profile** on a row opens a focus-trapped drawer that fetches the full identity (name + email + cover note + resume metadata) only on demand — the list response is anonymized. Notes + stage timeline live in the same drawer. |
...
| **Resume upload + autofill**     | Job detail page → **Apply now**. The form has a real file picker (PDF / DOC / DOCX, 15 MB cap) that uploads to MinIO via the backend, extracts text (pypdf for PDF, python-docx for DOCX), and returns autofill suggestions. The form pre-fills **matched skills** (cross-referenced against the job's required-skills list, so we suggest the *right* vocabulary, not every tech term in the document) and a **best-effort years-of-experience guess**, with a "Pre-filled from your resume — please review" banner so the candidate sees where the values came from. Legacy `.doc` uploads work but skip text extraction (documented behaviour). |
| Apply to a job                   | The form also captures cover note, current/expected CTC, notice period, and any skills you want to add on top of what the resume yielded. Duplicate applications are rejected at the API; jobs whose deadline has passed reject new applications. Submission is gated until a resume has finished uploading. |
